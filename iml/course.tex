...
\textbf{Goals:}
\begin{itemize}
    \item Discover the data manifold
    \item Obtain a compact, low-dimensional representation
    \item Remove noise if possible
\end{itemize}

General mapping: \(y_i = f(x_i)\) \quad — \quad Linear mapping: \(y_i = W^T x_i\)
\end{defnbox}
...
\subsection{Manifold Intuition}

\begin{rembox}[title=Manifold Hypothesis]
Although data lives in \(\R^D\), it often concentrates near a low-dimensional manifold.

\textbf{Example:} images of the digit 3 lie in a high-dimensional space \(32 \times 32\), but have only 3 degrees of freedom: 2 translations and 1 rotation.\end{rembox}
...
PCA finds directions with large variance in order to keep most of the important signal and remove noise.
\end{propbox}
