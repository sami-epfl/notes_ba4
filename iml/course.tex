...
\begin{defnbox}[title=Clustering]
An unsupervised task: partition \(N\) unlabelled samples into groups such that
\begin{itemize}
  \item points within the same cluster are similar,
  \item points in different clusters are dissimilar.
\end{itemize}
\end{defnbox}

\begin{defnbox}[title=K-Means Problem]
Given \(\{x_i\}_{i=1}^N \subset \R^D\) and K the number of clusters \(K\) group the samples into K different groups.

\textbf{Note:} In the idea of clusters, the distance between points within a cluster is small, and between clusters is big.
\end{defnbox}

\begin{algbox}[title=K-Means Algorithm]
\begin{enumerate}
    \item \textbf{Initialize} {\(\mu_1,\ldots,\mu_K\)} (randomly).
    \item \textbf{While not converged}
    \begin{itemize}
        \item \textbf{Step 2.1: Assign each point \(x_i\) to the nearest center \(\mu_k\)}

        \begin{itemize}
            \item For each point \(x_i\), compute the Euclidean distance to every $\{\mu_1, \dots, \mu_K\}$
            \item Find the smallest distance
            \item The point is said to be assigned to the corresponding cluster
            (note that each point is assigned to a single cluster)
        \end{itemize}

        \item \textbf{Step 2.2: Update each center \(\mu_k\) based on the points assigned to it}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The algorithm iteratively updates the cluster assignment for each point and
        the cluster centers. We need to eventually stop iterating
        
        \item This could be achieved by a fixed number of iterations. However, setting this
        number is arbitrary and a too small number can lead to bad results
        
        \item The algorithm is guaranteed to converge to a stable solution, where the
        cluster centers and the point assignments are fixed, i.e., do not change as
        more iterations are performed
        
        \item The difference in assignments or center locations between two iterations can
        be used as criteria to stop the algorithm
    \end{itemize}
    
    \item \textbf{Importantly:} While the algorithm always converges, it does
    not always converge to the best (desired) solution
    
    \begin{itemize}
        \item The solution depends on the center initialization
    \end{itemize}
\end{itemize}
\end{algbox}

\begin{warnbox}[title=Importance of Feature Scaling]
K-Means uses Euclidean distance, so features with large numerical ranges dominate.
Always \textbf{standardize} features (zero mean, unit variance) before clustering,
or use a distance function appropriate to the data type.
\end{warnbox}
