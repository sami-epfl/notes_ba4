...
% ------------------------------------------------------------
\newpage \section{Parametric feature expansion}
% ------------------------------------------------------------
...
Pour cela on va utiliser la \textbf{PARAMETRIC FEATURE EXPANSION!!!!}

\vspace{0.8cm}

\subsection{PARAMETRICsss FEATURE EXPANSION}
...
\[
z^{(1)} = f\!\left(a^{(1)}_{(1)}, a^{(1)}_{(2)}, \ldots \right)
\]

\vspace{0.4cm}

$z$ est ici the hidden representation et chaque élément $f\!\left(a_{(k)}^{(1)}\right)$ une hidden unit.
\vspace{0.6cm}

Par conséquent, avec ces changement on a :
...
\[
\Rightarrow \text{but has problems with jumps as } i \text{ obtain a more reliable gradient estimate, SGD with mini-batches of } B \text{ examples}
\]
