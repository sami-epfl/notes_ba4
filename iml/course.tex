...
\textbf{Example:} images of the digit 3 have only 3 degrees of freedom (2 translations + 1 rotation) despite living in \(\R^{572448}\).
\end{rembox}
...
Maximize the variance of the projected data. Equivalently, minimize the reconstruction error among all orthogonal projections.
\end{propbox}
...
\[
\frac{\partial L}{\partial w_{(1)}} = 2Cw_{(1)} - 2\lambda_1 w_{(1)} = 0
...
\[
W = \begin{bmatrix} w_{(1)} & w_{(2)} & \cdots & w_{(d)} \end{bmatrix} \in \R^{D \times d},
\qquad W^T W = I_d
\]
...
Among all orthogonal projections onto a \(d\)-dimensional subspace, PCA achieves the minimum average reconstruction error.
\end{propbox}
...
Map inputs to a (possibly infinite-dimensional) feature space:
\[
...
\[
Ka = \lambda_1 N a, \qquad a \in \R^N
\]
\end{propbox}
...
\subsection{Linear PCA Form}

\[
y = W^T(x-\bar{x}), \qquad \hat{x} = \bar{x} + Wy
\]
...
\textbf{Encoder:} \(x \mapsto y = f(W^T x)\) \qquad \textbf{Decoder:} \(y \mapsto \hat{x} = Wy\) \qquad \textbf{Goal:} \(\hat{x} \approx x\)
\end{defnbox}
...
\textbf{Encoder:} first half of layers \(x \to \cdots \to y\) \qquad \textbf{Decoder:} mirrored second half \(y \to \cdots \to \hat{x}\)
\end{defnbox}
...
\begin{itemize}
    ...
    \item Backpropagation computes gradients through encoder and decoder
\end{itemize}
\end{algbox}
...
\begin{itemize}
    ...
    \item Anomaly detection (high reconstruction error \(\Rightarrow\) anomaly)
    \item Sample generation (decode random latent codes)
\end{itemize}
\end{keybox}
