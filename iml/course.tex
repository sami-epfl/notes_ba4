...
En prenant M très grand, on remarque que la fonction devient de plus en plus précise, mais il se passe quoi si on ajoute des couches intermédiaires ?
...
We call going from $x$ to $y$ the \textbf{forward pass} of the network. Finally:
...
\begin{itemize}
    \item This is not very convenient, and it is easier to simply think of $\hat{y}$ as a function of all parameters
    \[
    \hat{y} = \hat{y}(\mathbf{W})
    \]
    where $\mathbf{W} = \left\{ W_{(l)} \right\}$ encompasses the parameters of all the layers
\end{itemize}
...
\end{minipage}
For regression, the empirical risk is typically defined using the square loss:
...
\begin{itemize}
    ...
    \item $\hat{y}_i(W)$ is the prediction of the network for sample $i$,
    \item $y_i$ is the target value.
\end{itemize}

\medskip

Because the empirical risk is an average over the training samples, its gradient is also an average of gradients:
...
Thus, to compute the gradient of the full empirical risk, we can focus on the gradient of a single loss term $\ell_i$.

\medskip

For a neuron $k$ in layer $l$, define the pre-activation value:
...
which corresponds to the weighted sum of the outputs from the previous layer.
...
measures how sensitive the loss is to the pre-activation of neuron $(k,l)$.

\medskip

Since $a^{(k)}_{(l)}$ depends linearly on the weights, we have:
...
Finally, by applying the chain rule:
...
To go back we will use a special algorithm \textbf{BACK PROPAGATION!!!!}
